 ...
 * @since [*next-version*]
 */
class MakeCapableMapTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Collection\MakeCapableMapTrait';
    ...
    public function testMake()
    {
        ...
        $config = [MapFactoryInterface::K_DATA => $data];
        ...
        $subject->expects($this->exactly(1))
            ...
            ->with($config, MapFactoryInterface::K_DATA)
        ...
        $subject->expects($this->exactly(1))
            ...
            ->with($config, $this->equalTo((object) $data))
            ->will($this->returnArgument(1));

        $result = $_subject->_make($config);
        $this->assertEquals((object) $data, $result, 'Wrong result produced');
    }
}
